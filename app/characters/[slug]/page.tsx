...
export default async function Character({
  ...
  const research = characterResearch(character.id),
    ...
    initials = character.name
      ...
      .map((part) => part[0])
      .join("");
  const isAntagonist = antagonistStatus.characters[character.id]?.isAntagonist,
    artVariant = characters.findIndex((item) => item.id === character.id) % 4;
  const visualClass = isAntagonist
    ...
    : undefined;
  const art = characterDisplayArt(
      character.name,
      Boolean(isAntagonist),
      visualClass,
    ),
    artStyle = { "--entity-art": `url('${art.url}')` } as CSSProperties;
  return (
    <main
      className={`identity-page entity-art display-art ${art.individual ? "individual-art" : "placeholder-art"} ${isAntagonist ? "antagonist-art" : "character-art"} art-variant-${artVariant}`}
      style={artStyle}
    >
      <section className="identity-hero">
        ...
        <div className="identity-art-note">
          {art.credit.toUpperCase()} / NOT CANONICAL EVIDENCE
        </div>
      ...
      <section className="identity-body">
        <div className="identity-abstract">
          {intro ? (
            <>
              <span>
                {intro.status.replaceAll("-", " ").toUpperCase()} ABSTRACT
              </span>
              <p>{intro.text}</p>
              <a href={intro.url} target="_blank" rel="noreferrer">
                {intro.provider}
                {intro.license ? ` · ${intro.license}` : ""}
              ...
              {intro.attribution && (
                <small>
                  {intro.attribution} {intro.changes}
                </small>
              )}
            </>
          ) : (
      ...
      {paths && (
        <section className="identity-ledger research-paths">
          <header>
            <span>RESEARCH PATHS</span>
            <h2>
              READ
              <br />
              <i>FURTHER.</i>
            </h2>
            <b>{paths.researchSources.length.toString().padStart(2, "0")}</b>
          </header>
          <div>
            {paths.researchSources.map((source, index) => (
              <article
                className="identity-claim"
              ...
              >
                <small>{String(index + 1).padStart(2, "0")}</small>
                <div>
                  <span>{source.sourceClass.replaceAll("-", " ")}</span>
                  <p>{source.sourceTitle}</p>
                  <small>{source.note}</small>
                </div>
                <aside>
                  <span
                    className={`status ${source.verification === "official-index" || source.verification === "secondary-verified" ? "secondary" : "provisional"}`}
                  >
                    {source.verification.replaceAll("-", " ")}
                  </span>
          ...
          <div className="identity-gap">
            <b>STARTING POINT, NOT A CONCLUSION</b>
            <p>
              The abstract introduces the record. These links expose the
              evidence, community documentation and unresolved paths from which
