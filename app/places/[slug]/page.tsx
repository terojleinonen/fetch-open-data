...
export default async function PlaceDetail({
  ...
  const linked = placeWorks(p),
    research = placeResearchPaths(p.id);
  const reading =
    p.mapStatus === "exact"
  ...
  const artStyle =
    p.kind === "fictional"
      ? ({
          "--place-art": `url('/art/places/${p.id}-v1.jpeg')`,
        } as CSSProperties)
      : undefined;
  return (
    <main className={`place-record place-art-${p.id}`} style={artStyle}>
      ...
      {research && (
        <section className="identity-ledger research-paths">
          <header>
            ...
            <b>
              {(research.researchSources.length + 1)
                .toString()
                .padStart(2, "0")}
            </b>
          </header>
          <div>
            <article className="identity-claim">
              <small>01</small>
              <div>
              ...
              <aside>
                <span className="status secondary">reviewed link</span>
                <a href={p.source.url} target="_blank" rel="noreferrer">
                  {p.source.name} ↗
                </a>
              </aside>
            </article>
            {research.researchSources.map((source, index) => (
              <article
          ...
          <div className="identity-gap">
            <b>MAPS ARE CLAIMS</b>
            <p>
              The opening summary is an orientation. Follow these records to
              inspect interpretations, community documentation and unresolved
              geographic evidence.
            </p>
          </div>
        </section>
